...
from graphrag.query.structured_search.global_search.prompts import (
    QUERY_PROMPT1, QUERY_PROMPT2
)
    ...
    async def _map_response_single_batch(
        ...
        **llm_kwargs,
        ...
        try:
            ...
            converted_input = convert_step1_input(context_data)
            search_prompt = QUERY_PROMPT1.format(query=query, input_text=converted_input)
            ...
            async with self.semaphore:
                ...
                original_output = search_response
                converted_output = convert_step1_output(search_response)
                if len(converted_output) == 0:
                    processed_response = [{"answer": "not avaliable", "score": 0}]
                else:
                    processed_response = converted_output
        ...
        **llm_kwargs,
        ...
        try:
            ...
            for point in filtered_key_points:
                ...
                formatted_response_text = f'<analyst>\n{point["answer"]}\n<score>{point["score"]}<\score>\n</analyst>'
            ...
            # )
            search_prompt = QUERY_PROMPT2.format(query=query, report_data=text_data)
...
# 240805 convert input of step 1 to xml format
import re
import xml.etree.ElementTree as ET
from xml.dom import minidom


def extract_records(input_text):
...
def create_xml(records):
    ...
    for record in records:
        ...
        ET.SubElement(rec, "id").text = record[0]
        ET.SubElement(rec, "title").text = record[1]
        ET.SubElement(rec, "content").text = '\n' + record[2] + '\n'

        rough_string = ET.tostring(rec, 'utf-8')
        reparsed = minidom.parseString(rough_string)
        pretty_xml = reparsed.toprettyxml(indent="", newl="\n")
        pretty_xml = pretty_xml[pretty_xml.index('?>') + 2:].strip()  # Remove the XML declaration

        xml_records.append(pretty_xml)

    return "\n\n".join(xml_records)


def convert_step1_input(original_data):
    records = extract_records(original_data)
    xml_string = create_xml(records)
    return xml_string


def convert_step1_output(llm_response):
    result = {}
    points = []
    try:
        ...
        for block in point_blocks:
            try:
                ...
                if title and content and ref and score:
                    score = int(score)
